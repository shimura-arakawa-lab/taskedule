...
import todoList2 from './api/todosList_v2.json';
...
function App ( {navigation} ) {
  ...
  const handleAdd = () => {
    ...
    changeMode('list');
    Puyos.push(
      <Puyo num={todos.length}/>
    );
    // const fs = require('fs');
    // fs.writeFileSync('./api/todosList_v2.json', JSON.stringify(todos));
  }
  ...
  const deleteTodo = (id: number) => {
    ...
    Puyos = Puyos.filter(Puyo => Puyo.id !== id);
  }
